 ...
 *
 * Runtime: Hermes Agent (uses read_file, write_file, terminal via the
...
// ─── graphify handler (Hermes-native knowledge graph) ───────────────────
...
function agentSkillsHandler(agentType, cwd, raw) {
  ...
  // Agents live in ~/.hermes/agents/ as flat .md files
  const agentsDir = path.join(hermesHome, 'agents');
  const agentFile = path.join(agentsDir, agentType + '.md');
  
  // Also check skills/hsd/ for skill-type agents
  const skillsDir = path.join(hermesHome, 'skills', 'hsd');
  const skillAgentFile = path.join(skillsDir, agentType, 'SKILL.md');
  
  const file = fs.existsSync(agentFile) ? agentFile :
               fs.existsSync(skillAgentFile) ? skillAgentFile : null;
  
  if (!file) {
    const available = [];
    if (fs.existsSync(agentsDir)) {
      available.push(...fs.readdirSync(agentsDir).filter(f => f.endsWith('.md')).map(f => f.replace('.md', '')));
    }
    return {
      error: `Agent skill not found: ${agentType}`,
      hermes_instruction: `Use skill_view(name="${agentType}") to load the skill. Agents live at ~/.hermes/agents/<name>.md`,
      available_agents: available.slice(0, 30),
    };
  }
  ...
  return {
    ...
    skill_path: file,
    hermes_instruction: `skill_view(name="${agentType}") loads the full agent definition. Use delegate_task with agent context for subagent spawning.`,
    skill_content_preview: content ? content.slice(0, 800) : null,
  };
}
...
function skillManifestHandler(cwd, raw) {
  const skillsDir = path.join(process.env.HERMES_HOME || path.join(require('os').homedir(), '.hermes'), 'skills', 'hsd');
  if (!fs.existsSync(skillsDir)) return { skills: [], count: 0 };
  const skills = [];
  for (const dir of fs.readdirSync(skillsDir, { withFileTypes: true })) {
    ...
    const fm = content ? (content.match(/^---\r?\n([\s\S]*?)\r?\n---/) || [])[1] : '';
    const name = fm ? (fm.match(/^name:\s*(.+)$/m) || [])[1] || dir.name : dir.name;
    const desc = fm ? (fm.match(/^description:\s*(.+)$/m) || [])[1] || '' : '';
    skills.push({ name: name.replace(/^["']|["']$/g, ''), dir: dir.name, description: desc.replace(/^["']|["']$/g, '') });
  }
